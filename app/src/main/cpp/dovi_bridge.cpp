// JNI bridge to libdovi (dovi_tool's C API) for runtime Dolby Vision profile 7 → 8.1
// RPU conversion. Adapted from NuvioMedia/NuvioTV (GPLv3) for com.brouken.player.
//
// Builds in two modes controlled by the CMake option DOVI_ENABLE_LIBDOVI:
//   - real  (DOVI_REAL_LINKED=1): links libdovi.a and performs the conversion.
//   - stub  (DOVI_REAL_LINKED=0): compiles without libdovi, conversion returns null.
// The stub keeps the app building even when the prebuilt libdovi archives are absent.
...
Java_com_brouken_player_dv_DoviBridge_nativeGetBridgeVersion(JNIEnv* env, jclass /* clazz */) {
#if DOVI_REAL_LINKED
    return env->NewStringUTF("dovi-bridge-libdovi-capi-1.0");
#else
    return env->NewStringUTF("dovi-bridge-stub-1.0");
#endif
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_brouken_player_dv_DoviBridge_nativeIsConversionPathReady(
    JNIEnv* /* env */,
    jclass /* clazz */
) {
#if DOVI_REAL_LINKED
...
Java_com_brouken_player_dv_DoviBridge_nativeConvertDv7RpuToDv81(
    JNIEnv* env,
    jclass /* clazz */,
    jbyteArray payload,
    jint mode
    ...
    dovi_data_free(out_data);
    dovi_rpu_free(rpu);
    // No per-frame log here: it runs once per frame and would flood logcat.
    return out;
#else
